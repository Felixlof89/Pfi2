...
public class Parser {
	/**
	 * Calls Sk�netrafiken API and searches for stations containing a String Use
	 * this Url to test from a browser: More information
	 * 
	 * @param serachStart
	 *            string to search for
	 * @return list of stations that fulfills the search criteria. Always
	 *         returns a number of central stations like Malm� Copenhagen etc Ok
	 *         testar
	 * */
	public static List<Station> getStationsFromURL(String searchStart) {
		...
		String xml = parser.getXmlFromUrl(Constants.baseURL
				+ Constants.getStationURL + searchStart);
		if (xml != null) {
	 ...
	 * @param serachURL
	 *            string to search for
	 * @return list of juoreys leaving the specified station. Always returns a
	 *         number of central stations like Malm� Copenhagen etc
	 * */
	public static Journeys getJourneys(String searchURL) {
		...
		if (xml != null) {
			...
			for (int i = 0; i < nl.getLength(); i++) {
				...
				String travelMinutes = Helpers.getTravelTimeinMinutes(
						depTimeString, arrTimeString);
				...
				thisJourney.setTimeToDeparture(Helpers
						.timeToDeparture(depTimeString));
				thisJourney.setNoOfZones(parser.getValue(e, "NoOfZones"));
				thisJourney.setLineTypeName(parser.getValue(e,
						"TransportModeName"));
				thisJourney.setArrTimeDeviation(parser.getValue(e,
						"ArrTimeDeviation"));
				thisJourney.setDepTimeDeviation(parser.getValue(e,
						"DepTimeDeviation"));
